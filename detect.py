    ...
    while offset < nf:
        ...
        if use_bd2:
            ...
            for p in preds:
                ...
                # AR spectral-shape features from the call's audio segment,
                # high-passed into the bat band so the fit tracks the call
                # rather than low-frequency background.
                from features import ar_features_band
                _ar1, _ar2, _ar1c = ar_features_band(
                    mono[int(t0_rel * sr):int(t1_rel * sr)], sr, FREQ_LOW)
